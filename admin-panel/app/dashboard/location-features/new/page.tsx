...
export default function NewLocationFeaturePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await locationFeaturesAPI.createFeature(formData)
      
      toast.success('تم إنشاء ميزة الموقع بنجاح')
      router.push('/dashboard/location-features')
    } catch (error: any) {
      console.error('خطأ في إنشاء ميزة الموقع:', error)
      toast.error(error.response?.data?.message || error.message || 'فشل في إنشاء ميزة الموقع')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold">إضافة ميزة موقع جديدة</h1>
          <p className="text-muted-foreground">إنشاء ميزة موقع جديدة في النظام</p>
        </div>
      </div>
